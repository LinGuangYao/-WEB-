...
namespace WebApplication12.ashx
{
    ...
    /// WuMaiChaXun 的摘要说明
    /// </summary>
    public class WuMaiChaXun : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            DataSet ds;
            string dt = Convert.ToString(context.Request["DAY"]);//接收前台传来的DAY参数       
            ...
            {
                OracleCommand cmd = new OracleCommand("select dianming,aqi,no2,pm25,pm10,so2,o3,co from  " + gkd + "  where riqi=to_date(' " + dt + " ' ,'yyyy-mm-dd')", conn);
                sda = new OracleDataAdapter();
                sda.SelectCommand = cmd;
                ds = new DataSet();
                sda.Fill(ds, "WMXX");
            ...
            {
                OracleCommand cmd = new OracleCommand("select dianming, " + wrw + " from  " + gkd + "  where riqi=to_date(' " + dt + " ' ,'yyyy-mm-dd')", conn);
                sda = new OracleDataAdapter();
                sda.SelectCommand = cmd;
                ds = new DataSet();
                sda.Fill(ds, "WMXX");
